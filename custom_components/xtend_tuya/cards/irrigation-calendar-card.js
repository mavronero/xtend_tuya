 ...
 */function ht(t){return dt({...t,state:!0,attribute:!1})}const pt={planned:0,missed:0,ran:1,unplanned:1,running:1};const ut=864e5,mt=36e5,gt="xt-irrigation-calendar-mode",vt="xt-irrigation-calendar-range";function ft(t){return new Date(t.getFullYear(),t.getMonth(),t.getDate())}function $t(t,e){const s=new Date(t);return s.setDate(s.getDate()+e),s}const _t=t=>String(t).padStart(2,"0"),bt=t=>{const e=new Date(t);return`${_t(e.getHours())}:${_t(e.getMinutes())}`},yt=t=>t>0?String(Math.round(t/6e4)):"–";function xt(t){const e=/·\s*~?([\d.,]+)\s*L\s*$/.exec(t);return e?Number(e[1].replace(",",".")):null}function wt(t,e){try{return localStorage.getItem(t)??e}catch{return e}}function At(t,e){try{localStorage.setItem(t,e)}catch{}}class kt extends ot{constructor(){super(...arguments),this._config=null,this._mode=["day","week","timeline"].includes(wt(gt,"day"))?wt(gt,"day"):"day",this._range=[1,3,7].includes(Number(wt(vt,"1")))?Number(wt(vt,"1")):1,this._anchor=ft(new Date),this._events=[],this._problemsOnly=!1,this._loading=!1,this._error=null,this._loadedKey=""}setConfig(t){this._config=t}getCardSize(){return 12}connectedCallback(){super.connectedCallback(),this._timer=window.setInterval(()=>this._load(!0),3e5)}disconnectedCallback(){super.disconnectedCallback(),this._timer&&window.clearInterval(this._timer)}updated(){const t=`${this._mode}|${this._range}|${this._anchor.getTime()}`;t!==this._loadedKey&&this.hass?.callApi&&(this._loadedKey=t,this._load())}_window(){if("week"===this._mode){const t=function(t){const e=ft(t);return e.setDate(e.getDate()-(e.getDay()+6)%7),e}(this._anchor);return[t,$t(t,7)]}const t=ft(this._anchor);return[t,$t(t,"timeline"===this._mode?this._range:1)]}_valveByRegistry(){const t=new Map;for(const e of this._config?.valves??[])t.set(e.registry_entity,e);return t}async _load(t=!1){if(!this.hass?.callApi)return;const[e,s]=this._window(),i=`?start=${encodeURIComponent(e.toISOString())}&end=${encodeURIComponent(s.toISOString())}`,n=this._config?.planned_entity??"calendar.irrigation_planned",r=this._config?.completed_entity??"calendar.irrigation_completed";t||(this._loading=!0),this._error=null;try{const[t,e]=await Promise.all([this.hass.callApi("GET",`calendars/${n}${i}`),this.hass.callApi("GET",`calendars/${r}${i}`)]),s=this._valveByRegistry(),o=(t,e)=>{const i=Date.parse(t.start.dateTime??t.start.date??"");let n=Date.parse(t.end.dateTime??t.end.date??"");if(!Number.isFinite(i))return null;(!Number.isFinite(n)||n<=i)&&(n=i+6e4);const r=(t.uid??"").split("#")[0],o=s.get(r);return{start:i,end:n,kind:e,key:r,name:o?.valve_name??t.summary.split(" · ")[0],summary:t.summary,liters:"planned"===e?null:xt(t.summary),path:o?.view_path}},a=(e??[]).map(t=>o(t,/Type: In progress/.test(t.description??"")?"running":"ran")).filter(t=>!!t),l=(t??[]).map(t=>o(t,"planned")).filter(t=>!!t);this._events=function(t,e,s,i=9e5){const n=new Set,r=[];for(const o of[...t].sort((t,e)=>t.start-e.start)){let t=null;for(const s of e){if(n.has(s)||s.key!==o.key)continue;const e=Math.abs(s.start-o.start);e<=i&&(!t||e<Math.abs(t.start-o.start))&&(t=s)}t?(n.add(t),r.push({...t,kind:"running"===t.kind?"running":"ran",planStart:o.start,planEnd:o.end,summary:`${t.summary??""}\nplanned ${o.summary??""}`})):o.end+i<s?r.push({...o,kind:"missed"}):r.push(o)}for(const t of e)n.has(t)||r.push("running"===t.kind?t:{...t,kind:"unplanned"});return r}(l,a,Date.now())}catch(t){this._error=function(t){const e=t;return e?.body?.message??e?.message??String(t)}(t)}finally{this._loading=!1,t||this.updateComplete.then(()=>this._scrollToFirst())}}_scrollToFirst(){if("timeline"===this._mode)return;const[t,e]=this._window();let s=null;for(const i of this._events){if(i.end<=t.getTime()||i.start>=e.getTime())continue;const n=new Date(Math.max(i.start,t.getTime())),r=n.getHours()+n.getMinutes()/60;(null===s||r<s)&&(s=r)}const i=this.renderRoot.querySelector(".scroll");i&&null!==s&&(i.scrollTop=Math.max(0,(s-1)*this._hourPx()))}_hourPx(){return this._config?.hour_height??56}_setMode(t){this._mode=t,At(gt,t)}_setRange(t){this._range=t,At(vt,String(t))}_shift(t){const e="week"===this._mode?7:"timeline"===this._mode?this._range:1;this._anchor=$t(this._anchor,e*t)}_open(t){if(!t)return;const e=window.location.pathname.split("/")[1]||"lovelace";window.history.pushState(null,"",`/${e}/${t}`),this.dispatchEvent(new Event("location-changed",{bubbles:!0,composed:!0}))}_rangeLabel(){const[t,e]=this._window(),s={weekday:"short",day:"numeric",month:"short"};return e.getTime()-t.getTime()<=ut?t.toLocaleDateString(void 0,s):`${t.toLocaleDateString(void 0,{day:"numeric",month:"short"})} – ${$t(e,-1).toLocaleDateString(void 0,s)}`}_countsText(){const t={planned:0,ran:0,missed:0,unplanned:0,running:0};for(const e of this._events)t[e.kind]++;const e=[`${t.ran} ran`,`${t.missed} missed`,`${t.planned} ahead`];return t.unplanned&&e.push(`${t.unplanned} unplanned`),t.running&&e.push(`${t.running} running`),e.join(" · ")}render(){if(!this._config)return W;const t=this._mode,e=(e,s)=>B`<button class="txt tab ${t===e?"on":""}" @click=${()=>this._setMode(e)}>${s}</button>`;return B`
    ...
    `}_renderGrid(){const[t]=this._window(),e="day"===this._mode?1:7,s=this._hourPx(),i=ft(new Date).getTime();return B`
      <div class="scroll">
        <div class="grid" style="--hour:${s}px;--days:${e}">
          ...
          ${Array.from({length:e},(n,r)=>{const o=$t(t,r).getTime(),a=o+ut,l=this._events.filter(t=>t.start<a&&t.end>o).map(t=>({...t,start:Math.max(t.start,o),end:Math.min(Math.max(t.end,t.start+9e5),a)})),c=function(t){const e=[...t].sort((t,e)=>t.start-e.start||pt[t.kind]-pt[e.kind]||t.name.localeCompare(e.name)),s=[];let i=[],n=[],r=-1/0;const o=()=>{const t=i.length;for(const e of n)e.lanes=t;n=[],i=[]};for(const t of e){t.start>=r&&o();let e=i.findIndex(e=>e<=t.start);-1===e?(e=i.length,i.push(t.end)):i[e]=t.end;const a={ev:t,lane:e,lanes:0};n.push(a),s.push(a),r=Math.max(r,t.end)}return o(),s}(l),d=c.reduce((t,e)=>Math.max(t,e.lanes),1),h=e>1?40:72;return B`
              <div
                class="col ${o===i?"today":""}"
    ...
    `}_renderTimeline(){const[t,e]=this._window(),s=t.getTime(),i=e.getTime()-s,n=t=>(t-s)/i*100,r=Date.now(),o=new Map;for(const t of this._events)(o.get(t.key)??o.set(t.key,[]).get(t.key)).push(t);const a=this._config?.valves??[],l=t=>`${t.home||"Unassigned"} · ${t.room||"—"}`,c=new Map;for(const t of[...a].sort((t,e)=>l(t).localeCompare(l(e))||t.valve_name.localeCompare(e.valve_name)))(c.get(l(t))??c.set(l(t),[]).get(l(t))).push(t);const d=[...c.keys()],h=1===this._range?3:3===this._range?12:24,p=[];for(let t=s;t<e.getTime();t+=h*mt){const e=new Date(t);p.push({left:n(t),label:h>=24||0===e.getHours()&&this._range>1?e.toLocaleDateString(void 0,{weekday:"short"}):`${_t(e.getHours())}`})}const u=Array.from({length:this._range-1},(t,e)=>n(s+(e+1)*ut)),m=r>s&&r<e.getTime()?n(r):null;let g=0;const v=d.map(t=>{const e=c.get(t).map(t=>{const e=o.get(t.registry_entity)??[],s=e.some(t=>"missed"===t.kind||"unplanned"===t.kind||t.end-t.start>144e5);return{v:t,evs:e,problem:s}}).filter(t=>!this._problemsOnly||t.problem);return e.length?(g+=e.length,B`
        <div class="grouphdr">${t}</div>
        ${e.map(({v:t,evs:e,problem:s})=>{const i=e.reduce((t,e)=>"planned"===e.kind||"missed"===e.kind?t+(e.end-e.start):null!=e.planStart&&null!=e.planEnd?t+(e.planEnd-e.planStart):t,0),r=e.filter(t=>"ran"===t.kind||"unplanned"===t.kind||"running"===t.kind),o=r.reduce((t,e)=>t+(e.end-e.start),0),a=r.reduce((t,e)=>null==e.liters?t:(t??0)+e.liters,null);return B`
            <div class="row clickable ${s?"problem":""}" @click=${()=>this._open(t.view_path)}>
              ...
              <div class="metric ${i?"":"muted"}">${yt(i)}</div>
              <div class="metric ${o?"":"muted"}">${yt(o)}</div>
              <div class="metric ${null==a?"muted":""}">${(t=>null==t?"–":String(Math.round(t)))(a)}</div>
            </div>
          `})}
      `):W});return B`
      <div class="tl">
