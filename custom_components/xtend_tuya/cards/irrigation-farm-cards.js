...
`,Wt=[["all","All"],["watering","Watering"],["attention","Attention"],["offline","Offline"]];class Bt extends dt{constructor(){super(...arguments),this.sites=[],this.value=Ot,this.counts={},this.statuses=!0}_set(t){this.value={...this.value,...t},this.dispatchEvent(new CustomEvent("xt-filter-changed",{detail:this.value,bubbles:!0,composed:!0}))}render(){const t=[...this.sites].map(t=>({id:t.id,path:Ut(this.sites,t.id)})).sort((t,e)=>t.path.localeCompare(e.path));return B`
  ...
  `],t([ht({attribute:!1})],fe.prototype,"summary",void 0),t([ht({type:Boolean})],fe.prototype,"editable",void 0),customElements.get("xt-mp-card")||customElements.define("xt-mp-card",fe);const xe={watering:"Watering",idle:"Idle",offline:"Offline"};function be(t){return new Date(t).toLocaleDateString(void 0,{day:"numeric",month:"short",year:"numeric"})}class _e extends dt{constructor(){super(...arguments),this.sites=[],this.valves=[],this.busy=!1,this.error=null}_fire(t,e){this.dispatchEvent(new CustomEvent(t,{detail:e,bubbles:!0,composed:!0}))}_save(t){t.preventDefault();const e=t.target,i=t=>e.elements.namedItem(t).value,s=i("expected_lpm").trim();this._fire("xt-mp-save",{name:i("name"),description:i("description"),expected_lpm:""===s?null:Number(s),site_id:i("site_id")||null})}_valveLine(t,e){return B`<div class="valve">
        ...
        ${s.length?B`<section>
              ...
              <ul>
                ${s.map(t=>B`<li>
                    ...
                    <span class="dim"
                      >${t.begin?be(t.begin):"from the start"} – ${t.end?be(t.end):"now"}</span
                    >
                  </li>`)}
              </ul>
            </section>`:G}
    ...
    `],t([ht({attribute:!1})],_e.prototype,"mp",void 0),t([ht({attribute:!1})],_e.prototype,"sites",void 0),t([ht({attribute:!1})],_e.prototype,"valves",void 0),t([ht({type:Boolean})],_e.prototype,"busy",void 0),t([ht()],_e.prototype,"error",void 0),customElements.get("xt-mp-editor")||customElements.define("xt-mp-editor",_e);const ye=r`
    :host {
      display: block;
    }
    .layout {
...
`,$e="xtend_tuya/irrigation_locations";function we(){return new URLSearchParams(window.location.search).get("site")}class ke extends dt{constructor(){super(...arguments),this._selected=we(),this._edit=!1,this._editing=null,this._busy=!1,this._error=null,this._farm=new Dt(this),this._onLocation=()=>{this._selected=we()}}setConfig(t){}getCardSize(){return 12}connectedCallback(){super.connectedCallback(),window.addEventListener("location-changed",this._onLocation),window.addEventListener("popstate",this._onLocation)}disconnectedCallback(){super.disconnectedCallback(),window.removeEventListener("location-changed",this._onLocation),window.removeEventListener("popstate",this._onLocation)}_open(t){const e=new URL(window.location.href);t?e.searchParams.set("site",t):e.searchParams.delete("site"),window.history.pushState(null,"",e.pathname+e.search),this._selected=t,this._error=null}async _post(t){if(!this.hass?.callApi)return!1;this._busy=!0,this._error=null;try{return await this.hass.callApi("POST",$e,t),await this._farm.refresh(!0),!0}catch(t){const e=t;return this._error=e.body?.error??e.message??String(t),!1}finally{this._busy=!1}}async _createSite(t,e){const i=t.querySelector("input");i.value.trim()&&await this._post({action:"create_site",name:i.value,parent_id:e})&&(i.value="")}async _saveSite(t,e){const i=t.querySelector("input").value,s=t.querySelector("select").value||null;await this._post({action:"update_site",id:e.id,name:i,parent_id:s})}async _deleteSite(t){await this._post({action:"delete_site",id:t.id})&&this._open(t.parent_id)}_header(t,e){const i=[{id:null,name:"All sites"}];if(t&&t.id!==le){const e=new Map(this._farm.data.sites.map(t=>[t.id,t])),s=[];for(let i=e.get(t.id);i&&s.length<20;i=i.parent_id?e.get(i.parent_id):void 0)s.unshift(i);i.push(...s.map(t=>({id:t.id,name:t.name})))}else t&&i.push({id:le,name:"No site"});const s=i[i.length-1].name;return B`<div class="header">
    ...
    `],t([ht({attribute:!1})],De.prototype,"buckets",void 0),t([ht()],De.prototype,"period",void 0),customElements.get("xt-pump-chart")||customElements.define("xt-pump-chart",De);class Oe extends dt{constructor(){super(...arguments),this.devices=[],this.exclude=[],this.placeholder="Search devices by name, area or model",this._q=""}render(){const t=this._q.trim().toLowerCase(),e=t?this.devices.filter(t=>!this.exclude.includes(t.id)).filter(e=>[e.name,e.area,e.model].some(e=>e?.toLowerCase().includes(t))).slice(0,30):[];return B`
    ...
    `}_pick(t){this._q="",this.dispatchEvent(new CustomEvent("xt-device-picked",{detail:t,bubbles:!0,composed:!0}))}}Oe.styles=[jt,r`
      :host {
        display: flex;
        flex-direction: column;
        gap: 4px;
    ...
    </section>`}_assignForm(t){const e=this._farm.data,i=e.sites.map(t=>({v:`site:${t.id}`,l:Ut(e.sites,t.id)})).sort((t,e)=>t.l.localeCompare(e.l)),s=e.locations.map(t=>({v:`location:${t.id}`,l:t.name})).sort((t,e)=>t.l.localeCompare(e.l));return B`<form class="editor" @submit=${e=>this._assign(e,t)}>
      <label
        >Assign this pump to
        <select>
    ...
    </form>`}_valveDevices(){return this._farm.valves.map(t=>this.hass?.entities[t.registry_entity]?.device_id).filter(t=>!!t)}_pumpForm(t){const e=this._deviceOf(t.meter_entity),i=[...new Set([...this._sensorsOf(e),...Re.map(e=>t[e]).filter(t=>!!t)])].sort();return B`<form class="editor" @submit=${e=>this._savePump(e,t)}>
      <label>Name <input name="name" .value=${t.name} required /></label>
      ${Re.map(e=>B`<label
          >${Ue[e]}
          <select name=${e}>
...
`;class di extends dt{setConfig(t){this._config=t}getCardSize(){return 3}_delay(t){const e=this._config?.rain_snow_delay,i=e?this.hass?.states[e]:void 0;if(!e||!i)return;const s=i.attributes,n=Number(s.min??0),a=Number(s.max??7),r=Math.min(a,Math.max(n,Number(i.state)+t*Number(s.step??1)));this.hass.callService("number","set_value",{entity_id:e,value:r})}render(){if(!this._config||!this.hass)return G;const t=this._config.sleep_mode?this.hass.states[this._config.sleep_mode]:void 0,e=this._config.rain_snow_delay?this.hass.states[this._config.rain_snow_delay]:void 0,i=e?Number(e.state):NaN,s=t=>!t||"unavailable"===t.state||"unknown"===t.state;return B`<ha-card>
    ...
    `],t([ht({attribute:!1})],di.prototype,"hass",void 0),t([mt()],di.prototype,"_config",void 0),customElements.get("irrigation-valve-settings-card")||customElements.define("irrigation-valve-settings-card",di);
