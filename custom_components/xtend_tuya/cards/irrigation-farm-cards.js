...
`,Wt=[["all","All"],["watering","Watering"],["attention","Attention"],["offline","Offline"]];class Ft extends dt{constructor(){super(...arguments),this.sites=[],this.value=Ot,this.counts={}}_set(t){this.value={...this.value,...t},this.dispatchEvent(new CustomEvent("xt-filter-changed",{detail:this.value,bubbles:!0,composed:!0}))}render(){const t=[...this.sites].map(t=>({id:t.id,path:Ut(this.sites,t.id)})).sort((t,e)=>t.path.localeCompare(e.path));return B`
  ...
  `],t([ht({attribute:!1})],Ft.prototype,"sites",void 0),t([ht({attribute:!1})],Ft.prototype,"value",void 0),t([ht({attribute:!1})],Ft.prototype,"counts",void 0),customElements.get("xt-valve-filter-bar")||customElements.define("xt-valve-filter-bar",Ft);const Bt=864e5;function qt(t){return new Date(t).toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"})}function Vt(t){return`${function(t,e=Date.now()){const i=new Date(t),s=new Date(e);s.setHours(0,0,0,0);const n=Math.floor((i.getTime()-s.getTime())/Bt);return 0===n?"Today":-1===n?"Yesterday":1===n?"Tomorrow":i.toLocaleDateString(void 0,{weekday:"short",day:"numeric",month:"short"})}(t)} ${qt(t)}`}function Gt(t){return`${Math.round(t).toLocaleString()} L`}function Kt(t){return t>=95?"mdi:battery":t<10?"mdi:battery-outline":"mdi:battery-"+10*Math.floor(t/10)}function Jt(t){return Math.abs(t)<1e3?`${Math.round(t)} L`:`${(t/1e3).toFixed(1)} m³`}class Zt extends dt{constructor(){super(...arguments),this.daily=[],this.unit="L"}render(){const t=Math.max(...this.daily,0),e=new Date;e.setHours(12,0,0,0);const i=this.daily.length;return this.daily.map((s,n)=>B`<span
          title="${(t=>new Date(e.getTime()-(i-1-t)*Bt).toLocaleDateString(void 0,{weekday:"short",day:"numeric",month:"short"}))(n)}: ${Math.round(s).toLocaleString()} ${this.unit}"
  ...
  `],t([ht({attribute:!1})],Zt.prototype,"daily",void 0),t([ht()],Zt.prototype,"unit",void 0),customElements.get("xt-week-bars")||customElements.define("xt-week-bars",Zt);const Xt={low_battery:"Low battery",stale:"No report 36 h",missed:"Missed",no_flow:"No water flow"},Yt={low_battery:"Battery below 20 %",stale:"The valve has not reported for more than 36 hours",missed:"A planned run in the last 24 hours did not happen",no_flow:"The last run measured no water"};function Qt(t){const e=[Vt(t.start),`${Math.round(t.minutes)} min`];return null!==t.liters&&e.push(`${Math.round(t.liters)} L`),e.join(" · ")}function te(t){return(t.number?t.name.replace(/\s*\(\d+\)\s*$/,""):t.name)||t.name}class ee extends dt{_open(){this.summary&&this.dispatchEvent(new CustomEvent("xt-valve-open",{detail:this.summary.view_path,bubbles:!0,composed:!0}))}_status(t){if("watering"===t.status){const e=null!==t.flow_lpm?` · ${t.flow_lpm.toFixed(1)} L/min`:"";return B`<span class="status watering" title="Watering now"><i></i>Watering${t.since?` since ${qt(t.since)}`:""}${e}</span>`}return"offline"===t.status?B`<span class="status offline" title="Not reachable"><i></i>Offline${t.since?` for ${function(t,e=Date.now()){const i=Math.round((e-t)/6e4);if(i<60)return`${i} min`;const s=Math.round(i/60);return s<48?`${s} h`:`${Math.round(s/24)} d`}(t.since)}`:""}</span>`:B`<span class="status idle" title="Online, not watering"><i></i>Idle</span>`}_week(t){const e=t.week.unit,i="L"===e?`${Math.round(t.week.liters)} L`:`${Math.round(t.week.minutes)} min`;return B`<div class="week">
      ...
      >
    </div>`}render(){const t=this.summary;if(!t)return G;const e=t.location&&t.location.name!==te(t)?t.location.name:null,i=t.location?[e,t.site?.name].filter(Boolean).join(" · "):"No location";return B`<ha-card class=${t.status} @click=${this._open} tabindex="0" role="link" aria-label=${t.name}>
      <div class="head">
        <span class="name" title=${t.name}>${te(t)}</span>
        ...
        ${null!==t.battery?B`<span class="battery ${t.badges.includes("low_battery")?"low":""}" title="Battery ${Math.round(t.battery)} %"
              ><ha-icon icon=${Kt(t.battery)}></ha-icon>${Math.round(t.battery)} %</span
            >`:G}
      </div>
      ...
      <dl>
        ...
        <dd title="Last run: start · duration${t.has_flow_meter?" · water":""}">${t.last?Qt(t.last):"–"}</dd>
        <dt title="Next planned run"><ha-icon icon="mdi:calendar-clock"></ha-icon></dt>
        <dd title="Next planned run: start · duration">${t.next?Qt(t.next):"–"}</dd>
      </dl>
      ${this._week(t)}
      ${t.badges.length?B`<div class="badges">
            ${t.badges.map(e=>B`<span class="badge ${e}" title=${Yt[e]}>${Xt[e]}${"missed"===e&&t.missed>1?` ${t.missed}`:""}</span>`)}
          </div>`:G}
    </ha-card>`}}ee.styles=[It,r`
    :host {
      display: block;
    }
    ha-card {
  ...
  `],t([ht({attribute:!1})],ee.prototype,"summary",void 0),customElements.get("xt-valve-card")||customElements.define("xt-valve-card",ee);class ie extends dt{constructor(){super(...arguments),this.collapsed=!1}render(){const t=this.section;return t?B`
  ...
  `,t([ht({attribute:!1})],ne.prototype,"hass",void 0),t([mt()],ne.prototype,"_config",void 0),t([mt()],ne.prototype,"_filter",void 0),customElements.get("irrigation-valves-card")||customElements.define("irrigation-valves-card",ne);const ae="__no_site__";function re(t,e){return t.filter(t=>t.parent_id===e).sort((t,e)=>t.name.localeCompare(e.name))}function oe(t,e,i){const s=t===ae,n=s?null:e.sites.find(e=>e.id===t)??null,a=s?null:Tt(e.sites,t),r=i.filter(t=>a?!!t.site_id&&a.has(t.site_id):!t.site_id),o=r.flatMap(t=>t.valves),l=[0,0,0,0,0,0,0];let d=0,c=0;for(const t of r)d+=t.week.runs,c+=t.week.liters,"L"===t.week.unit&&t.week.daily.forEach((t,e)=>l[e]+=t);const p=r.map(t=>t.last?.start).filter(t=>"number"==typeof t),u=r.map(t=>t.next?.start).filter(t=>"number"==typeof t),h=s?null:function(t,e){const i=new Map(t.sites.map(t=>[t.id,t]));for(let s=i.get(e),n=0;s&&n<20;s=s.parent_id?i.get(s.parent_id):void 0,n++){const i=t.pumpAssignments.find(t=>"site"===t.target_kind&&t.target_id===s.id&&null===t.end),n=i&&t.pumps.find(t=>t.id===i.pump_id);if(n)return{pump:n,via:s.id===e?null:s.name}}return null}(e,t);return{id:t,name:n?.name??"No site",path:n?Ut(e.sites,n.id):"No site",parent_id:n?.parent_id??null,children:s?[]:re(e.sites,t).map(t=>t.id),mps:r.length,valves:o.length,online:o.filter(t=>"offline"!==t.status).length,watering:o.filter(t=>"watering"===t.status).length,attention:r.filter(t=>"offline"!==t.status&&t.badges.length>0).length,offline:o.filter(t=>"offline"===t.status).length,last:p.length?Math.max(...p):null,next:u.length?Math.min(...u):null,week:{runs:d,liters:c,daily:l},pump:h?{name:h.pump.name,via:h.via}:null}}function le(t,e,i,s){const n=t.valves.map(t=>i.find(e=>e.device_id===t)).filter(t=>!!t),a=function(t,e){const i=[];for(const s of e.runs){const e=Date.parse(s.end),n=t.assignments.some(t=>t.device_id===s.device_id&&(null===t.begin||t.begin<=e)&&(null===t.end||e<t.end));n&&i.push({start:Date.parse(s.start),minutes:(s.duration_seconds??0)/60,liters:"number"==typeof s.liters?s.liters:null})}return i.sort((t,e)=>t.start-e.start)}(t,e),r=n.some(t=>t.has_flow_meter)||a.some(t=>null!==t.liters&&t.liters>0),o=a.filter(t=>t.start>=s-2592e6&&null!==t.liters&&t.minutes>=1),l=o.reduce((t,e)=>t+e.minutes,0),d=r&&l>0?o.reduce((t,e)=>t+(e.liters??0),0)/l:null,c=n.length?n.some(t=>"watering"===t.status)?"watering":n.every(t=>"offline"===t.status)?"offline":"idle":"empty",p=n.map(t=>t.next).filter(t=>!!t),u=a[a.length-1]??null,h=n.reduce((t,e)=>t+e.missed,0),m=[];t.valves.length||m.push("no_valve");for(const t of["low_battery","stale"])n.some(e=>e.badges.includes(t))&&m.push(t);if(h>0&&m.push("missed"),r&&u&&0===u.liters&&u.minutes>=1&&m.push("no_flow"),null!==d&&t.expected_lpm){const e=(d-t.expected_lpm)/t.expected_lpm;e<-.25?m.push("flow_low"):e>.25&&m.push("flow_high")}return{id:t.id,name:t.name,site_id:t.site_id,valves:n.map(t=>({device_id:t.device_id,number:t.number,name:t.name,status:t.status,battery:t.battery,view_path:t.view_path})),status:c,last:u,next:p.length?p.reduce((t,e)=>e.start<t.start?e:t):null,week:Lt(a,s,r),avg_lpm:d,expected_lpm:t.expected_lpm,pump:t.pump,missed:h,badges:m}}class de extends dt{_open(){this.summary&&this.dispatchEvent(new CustomEvent("xt-site-open",{detail:this.summary.id,bubbles:!0,composed:!0}))}render(){const t=this.summary;if(!t)return G;const e=t.watering?B`<span class="status watering" title="Valves watering now"><i></i>${t.watering} watering</span>`:B`<span class="status" title="Valves online / valves in this site"><i></i>${t.online} / ${t.valves} online</span>`;return B`<ha-card @click=${this._open} tabindex="0" role="link" aria-label=${t.path}>
      ...
      <div class="week">
        ...
        <span class="dim" title="Last 7 days: runs and water of all valves in this site"
          >${t.week.runs} runs · ${Gt(t.week.liters)}</span
        >
      </div>
  ...
  `],t([ht({attribute:!1})],de.prototype,"summary",void 0),customElements.get("xt-site-card")||customElements.define("xt-site-card",de);class ce extends dt{constructor(){super(...arguments),this.sites=[],this.selected=null,this.counts={},this.noSite=!1,this._closed=new Set}_open(t){this.dispatchEvent(new CustomEvent("xt-site-open",{detail:t,bubbles:!0,composed:!0}))}_toggle(t,e){t.stopPropagation();const i=new Set(this._closed);i.has(e)?i.delete(e):i.add(e),this._closed=i}_node(t,e){const i=re(this.sites,t.id),s=this._closed.has(t.id);return B`
      <button
    ...
    `}render(){return B`
      ...
      ${this.noSite?B`<button class="node ${this.selected===ae?"on":""}" @click=${()=>this._open(ae)}>
            <span class="chev"></span><span class="label dim">No site</span>
            <span class="count">${this.counts[ae]??""}</span>
          </button>`:G}
    `}}ce.styles=[It,r`
    :host {
      display: flex;
  ...
  `],t([ht({attribute:!1})],ce.prototype,"sites",void 0),t([ht({attribute:!1})],ce.prototype,"selected",void 0),t([ht({attribute:!1})],ce.prototype,"counts",void 0),t([ht({type:Boolean})],ce.prototype,"noSite",void 0),t([mt()],ce.prototype,"_closed",void 0),customElements.get("xt-site-tree")||customElements.define("xt-site-tree",ce);const pe={no_valve:["No valve","No valve is assigned to this metering point","warn"],low_battery:["Low battery","The valve's battery is below 20 %","warn"],stale:["No report 36 h","The valve has not reported for more than 36 hours","warn"],missed:["Missed","A planned run in the last 24 hours did not happen","bad"],no_flow:["No water flow","The last run here measured no water","bad"],flow_low:["Flow low","Mean flow of the last 30 days is well below the expected L/min","bad"],flow_high:["Flow high","Mean flow of the last 30 days is well above the expected L/min (leak?)","bad"]},ue={watering:"Watering",idle:"Idle",offline:"Offline"};function he(t){const e=[Vt(t.start),`${Math.round(t.minutes)} min`];return null!==t.liters&&e.push(`${Math.round(t.liters)} L`),e.join(" · ")}class me extends dt{constructor(){super(...arguments),this.editable=!1}_openValve(t,e){t.stopPropagation(),this.dispatchEvent(new CustomEvent("xt-valve-open",{detail:e.view_path,bubbles:!0,composed:!0}))}_valve(t){return B`<button class="valve ${t.status}" @click=${e=>this._openValve(e,t)} title="Open valve ${t.name}">
      ...
      <dl>
        ...
        <dd title="Last run here: start · duration · water">${t.last?he(t.last):"–"}</dd>
        <dt title="Next planned run"><ha-icon icon="mdi:calendar-clock"></ha-icon></dt>
        <dd title="Next planned run: start · duration">${t.next?he(t.next):"–"}</dd>
      </dl>
      <div class="week">
        <ha-icon icon="mdi:chart-bar" title="Last 7 days"></ha-icon>
        <xt-week-bars .daily=${t.week.daily} unit=${e}></xt-week-bars>
  ...
  `],t([ht({attribute:!1})],me.prototype,"summary",void 0),t([ht({type:Boolean})],me.prototype,"editable",void 0),customElements.get("xt-mp-card")||customElements.define("xt-mp-card",me);const ve={watering:"Watering",idle:"Idle",offline:"Offline"};function fe(t){return new Date(t).toLocaleDateString(void 0,{day:"numeric",month:"short",year:"numeric"})}class ge extends dt{constructor(){super(...arguments),this.sites=[],this.valves=[],this.busy=!1,this.error=null}_fire(t,e){this.dispatchEvent(new CustomEvent(t,{detail:e,bubbles:!0,composed:!0}))}_save(t){t.preventDefault();const e=t.target,i=t=>e.elements.namedItem(t).value,s=i("expected_lpm").trim();this._fire("xt-mp-save",{name:i("name"),description:i("description"),expected_lpm:""===s?null:Number(s),site_id:i("site_id")||null})}_valveLine(t,e){return B`<div class="valve">
      <ha-icon icon="mdi:valve"></ha-icon>
      <span class="num">${t?.label??e}</span>
      ${t?B`<i class=${t.status}></i><span>${ve[t.status]}</span>`:G}
      ${null!=t?.battery?B`<span class="dim"><ha-icon icon=${Kt(t.battery)}></ha-icon>${Math.round(t.battery)} %</span>`:G}
    ...
    `],t([ht({attribute:!1})],ge.prototype,"mp",void 0),t([ht({attribute:!1})],ge.prototype,"sites",void 0),t([ht({attribute:!1})],ge.prototype,"valves",void 0),t([ht({type:Boolean})],ge.prototype,"busy",void 0),t([ht()],ge.prototype,"error",void 0),customElements.get("xt-mp-editor")||customElements.define("xt-mp-editor",ge);const _e=r`
    :host {
      display: block;
    }
    .layout {
...
`,be="xtend_tuya/irrigation_locations";function xe(){return new URLSearchParams(window.location.search).get("site")}class $e extends dt{constructor(){super(...arguments),this._selected=xe(),this._edit=!1,this._editing=null,this._busy=!1,this._error=null,this._farm=new Pt(this),this._onLocation=()=>{this._selected=xe()}}setConfig(t){}getCardSize(){return 12}connectedCallback(){super.connectedCallback(),window.addEventListener("location-changed",this._onLocation),window.addEventListener("popstate",this._onLocation)}disconnectedCallback(){super.disconnectedCallback(),window.removeEventListener("location-changed",this._onLocation),window.removeEventListener("popstate",this._onLocation)}_open(t){const e=new URL(window.location.href);t?e.searchParams.set("site",t):e.searchParams.delete("site"),window.history.pushState(null,"",e.pathname+e.search),this._selected=t,this._error=null}async _post(t){if(!this.hass?.callApi)return!1;this._busy=!0,this._error=null;try{return await this.hass.callApi("POST",be,t),await this._farm.refresh(!0),!0}catch(t){const e=t;return this._error=e.body?.error??e.message??String(t),!1}finally{this._busy=!1}}async _createSite(t,e){const i=t.querySelector("input");i.value.trim()&&await this._post({action:"create_site",name:i.value,parent_id:e})&&(i.value="")}async _saveSite(t,e){const i=t.querySelector("input").value,s=t.querySelector("select").value||null;await this._post({action:"update_site",id:e.id,name:i,parent_id:s})}async _deleteSite(t){await this._post({action:"delete_site",id:t.id})&&this._open(t.parent_id)}_header(t,e){const i=[{id:null,name:"All sites"}];if(t&&t.id!==ae){const e=new Map(this._farm.data.sites.map(t=>[t.id,t])),s=[];for(let i=e.get(t.id);i&&s.length<20;i=i.parent_id?e.get(i.parent_id):void 0)s.unshift(i);i.push(...s.map(t=>({id:t.id,name:t.name})))}else t&&i.push({id:ae,name:"No site"});const s=i[i.length-1].name;return B`<div class="header">
      ...
      <div class="figures">
        ${t?B`
              ...
              <span title="Last 7 days"
                ><xt-week-bars .daily=${t.week.daily}></xt-week-bars>${t.week.runs} runs · ${Gt(t.week.liters)}</span
              >
    ...
    ></xt-mp-editor>`}_addMp(t){return B`<form class="editor" @submit=${e=>(e.preventDefault(),this._createMp(e.target,t))}>
      ...
      <button type="submit" ?disabled=${this._busy}>Add</button>
    </form>`}render(){if(!this.hass)return G;if(!this._farm.loaded)return B`<ha-card><div class="msg">Loading sites…</div></ha-card>`;const t=this._farm.data,e=this._farm.summaries(),i=Date.now(),s=t.locations.map(s=>le(s,t,e,i)),n=new Map(s.map(t=>[t.id,t])),a=t.locations.some(t=>!t.site_id),r=this._selected&&(this._selected===ae||t.sites.some(t=>t.id===this._selected))?this._selected:null,o=r&&r!==ae?t.sites.find(t=>t.id===r)??null:null,l=r?oe(r,t,s):null,d=r?l.children:[...re(t.sites,null).map(t=>t.id),...a?[ae]:[]],c=d.map(e=>oe(e,t,s)),p=r?t.locations.filter(t=>r===ae?!t.site_id:t.site_id===r).sort((t,e)=>t.name.localeCompare(e.name)):[],u={};for(const e of t.sites)u[e.id]=t.locations.filter(i=>Tt(t.sites,e.id).has(i.site_id??"")).length;u[ae]=t.locations.filter(t=>!t.site_id).length;const h=s.flatMap(t=>t.valves),m={sites:t.sites.length,mps:t.locations.length,valves:h.length,online:h.filter(t=>"offline"!==t.status).length},v=this._editing?t.locations.find(t=>t.id===this._editing):void 0;return B`<div
      class="layout"
    ...
    >
      ...
      <main>
        ...
        ${c.length||this._edit&&r!==ae?B`<h3>${r?"Sub-sites":"Sites"} <span class="count">${c.length}</span></h3>
              <div class="grid">${c.map(t=>B`<xt-site-card .summary=${t}></xt-site-card>`)}</div>
              ${this._edit&&r!==ae?this._addSite(r):G}`:G}
        ${p.length||this._edit&&r?B`<h3>Metering points <span class="count">${p.length}</span></h3>
              ...
              ${this._edit&&r?this._addMp(r===ae?null:r):G}`:G}
    ...
    </div>`}}$e.styles=[It,_e],t([ht({attribute:!1})],$e.prototype,"hass",void 0),t([mt()],$e.prototype,"_selected",void 0),t([mt()],$e.prototype,"_edit",void 0),t([mt()],$e.prototype,"_editing",void 0),t([mt()],$e.prototype,"_busy",void 0),t([mt()],$e.prototype,"_error",void 0),customElements.get("irrigation-sites-card")||customElements.define("irrigation-sites-card",$e);const ye=1e3;function we(t,e){return(null===t.begin||t.begin<=e)&&(null===t.end||e<t.end)}function ke(t,e,i){const s=(e,s)=>t.pumpAssignments.find(t=>t.target_kind===e&&t.target_id===s&&we(t,i))?.pump_id??null,n=s("location",e.id);if(n)return n;const a=new Map(t.sites.map(t=>[t.id,t]));for(let t=e.site_id?a.get(e.site_id):void 0,i=0;t&&i<20;i++){const e=s("site",t.id);if(e)return e;t=t.parent_id?a.get(t.parent_id):void 0}return null}function Ae(t,e,i){const s=t.pumpAssignments.filter(t=>t.pump_id===e&&"site"===t.target_kind&&we(t,i)).map(t=>t.target_id),n=[],a=[],r=e=>{const i=new Map(t.sites.map(t=>[t.id,t]));for(let t=e.site_id?i.get(e.site_id):void 0,n=0;t&&n<20;n++){if(s.includes(t.id))return!0;t=t.parent_id?i.get(t.parent_id):void 0}return!1};for(const s of t.locations){const o=ke(t,s,i);o===e?n.push(s.id):o&&r(s)&&a.push({mp:s.id,pump:o})}return{sites:s,mps:n,overridden:a}}function Se(t,e){const i=new Date(t);return i.setMinutes(0,0,0),"day"===e&&i.setHours(0),i.getTime()}const Ee="__no_pump__";class Ce extends dt{constructor(){super(...arguments),this.items=[],this.selected=null}render(){return this.items.map(t=>B`<button
        class="node ${t.id===this.selected?"on":""}"
        aria-current=${t.id===this.selected?"page":"false"}
        @click=${()=>this.dispatchEvent(new CustomEvent("xt-pump-open",{detail:t.id,bubbles:!0,composed:!0}))}
      >
        ${t.id===Ee?B`<span class="dot none"></span>`:B`<span class="dot ${t.state}" title=${"unknown"===t.state?"No data from the pump":t.state}></span>`}
        <span class="label ${t.id===Ee?"dim":""}">${t.name}</span>
        <span class="count">${t.count}</span>
      </button>`)}}Ce.styles=[It,r`
      :host {
        display: flex;
        flex-direction: column;
        gap: 1px;
    ...
    `],t([ht({attribute:!1})],Ce.prototype,"items",void 0),t([ht({attribute:!1})],Ce.prototype,"selected",void 0),customElements.get("xt-pump-list")||customElements.define("xt-pump-list",Ce);const Me=160;class Le extends dt{constructor(){super(...arguments),this.buckets=[],this.period="hour"}_label(t){const e=new Date(t);return"hour"===this.period?e.toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"}):e.toLocaleDateString(void 0,{weekday:"short",day:"numeric"})}render(){const t=this.buckets.length;if(!t)return G;const e=Math.max(1,...this.buckets.map(t=>Math.max(t.pump??0,t.valves+t.consumers))),i=100/t,s=t=>t/e*142,n=Math.ceil(t/8);return B`
      <svg viewBox="0 0 100 ${Me}" preserveAspectRatio="none" role="img" aria-label="Pump delivery against valve and consumer use">
        ${this.buckets.map((t,e)=>{const n=e*i,a=t.valves+t.consumers,r=`${this._label(t.start)}: pump ${null===t.pump?"no data":Jt(t.pump)}, valves ${Jt(t.valves)}${t.consumers?`, consumers ${Jt(t.consumers)}`:""}${null!==t.pump?`, unaccounted ${Jt(t.pump-a)}`:""}`;return q`<g>
            <title>${r}</title>
    ...
    `],t([ht({attribute:!1})],Le.prototype,"buckets",void 0),t([ht()],Le.prototype,"period",void 0),customElements.get("xt-pump-chart")||customElements.define("xt-pump-chart",Le);class Ne extends dt{constructor(){super(...arguments),this.devices=[],this.exclude=[],this.placeholder="Search devices by name, area or model",this._q=""}render(){const t=this._q.trim().toLowerCase(),e=t?this.devices.filter(t=>!this.exclude.includes(t.id)).filter(e=>[e.name,e.area,e.model].some(e=>e?.toLowerCase().includes(t))).slice(0,30):[];return B`
    ...
    `}_pick(t){this._q="",this.dispatchEvent(new CustomEvent("xt-device-picked",{detail:t,bubbles:!0,composed:!0}))}}Ne.styles=[It,r`
      :host {
        display: flex;
        flex-direction: column;
        gap: 4px;
    ...
    `],t([ht({attribute:!1})],Ne.prototype,"devices",void 0),t([ht({attribute:!1})],Ne.prototype,"exclude",void 0),t([ht()],Ne.prototype,"placeholder",void 0),t([mt()],Ne.prototype,"_q",void 0),customElements.get("xt-device-picker")||customElements.define("xt-device-picker",Ne);const Pe={"24h":{ms:864e5,period:"hour"},"7d":{ms:6048e5,period:"day"},"30d":{ms:2592e6,period:"day"}},Oe="xt-pumps-range",De=["meter_entity","flow_entity","pressure_entity","status_entity"],ze={meter_entity:"Water meter (m³ total)",flow_entity:"Flow rate",pressure_entity:"Pressure",status_entity:"Status"},Te={meter_entity:/_fct_total_delivered_flow_mc$/,flow_entity:/_vf_flowliter$/,pressure_entity:/_vp_pressurebar$/,status_entity:/_pumpstatus$/};function Ue(){return new URLSearchParams(window.location.search).get("pump")}class Re extends dt{constructor(){super(...arguments),this._selected=Ue(),this._range=function(){try{const t=localStorage.getItem(Oe);return t&&t in Pe?t:"7d"}catch{return"7d"}}(),this._stats={},this._edit=!1,this._busy=!1,this._error=null,this._connecting=null,this._statsKey="",this._farm=new Pt(this),this._onLocation=()=>{this._selected=Ue()}}setConfig(t){}getCardSize(){return 12}connectedCallback(){super.connectedCallback(),window.addEventListener("location-changed",this._onLocation),window.addEventListener("popstate",this._onLocation)}disconnectedCallback(){super.disconnectedCallback(),window.removeEventListener("location-changed",this._onLocation),window.removeEventListener("popstate",this._onLocation)}updated(){const t=`${this._range}|${this._farm.data.pumps.length}|${this._farm.data.pumpConnections.length}`;this.hass?.callApi&&this._farm.loaded&&t!==this._statsKey&&(this._statsKey=t,this._loadStats())}async _loadStats(){const{ms:t}=Pe[this._range],e=new Date(Date.now()-t).toISOString();try{const t=await this.hass.callApi("GET",`xtend_tuya/pump_stats?period=hour&start=${encodeURIComponent(e)}`);this._stats=t.series??{}}catch(t){this._error=`Could not load pump statistics: ${t instanceof Error?t.message:String(t)}`}}_state(t){return t?this.hass?.states[t]:void 0}_num(t){const e=this._state(t),i=Number(e?.state);return e&&""!==e.state&&Number.isFinite(i)?i:null}_pumpState(t){const e=this._state(t.status_entity)?.state,i=this._num(t.flow_entity),s=this._state(t.meter_entity)?.state;return s&&"unknown"!==s&&"unavailable"!==s?"Go"===e||null!==i&&i>0?"running":"idle":"unknown"}_devices(){const t=this.hass?.areas??{};return Object.values(this.hass?.devices??{}).map(e=>({id:e.id,name:e.name_by_user||e.name||e.id,area:e.area_id?t[e.area_id]?.name??null:null,model:e.model??null}))}_deviceName(t){const e=this.hass?.devices[t];return e&&(e.name_by_user||e.name)||t}_sensorsOf(t){return t&&this.hass?Object.values(this.hass.entities).filter(e=>e.device_id===t&&e.entity_id.startsWith("sensor.")).map(t=>t.entity_id).sort():[]}_deviceOf(t){return t?this.hass?.entities[t]?.device_id??null:null}_open(t){const e=new URL(window.location.href);t?e.searchParams.set("pump",t):e.searchParams.delete("pump"),window.history.pushState(null,"",e.pathname+e.search),this._selected=t,this._error=null,this._connecting=null}_setRange(t){this._range=t;try{localStorage.setItem(Oe,t)}catch{}}async _post(t){if(!this.hass?.callApi)return null;this._busy=!0,this._error=null;try{const e=await this.hass.callApi("POST","xtend_tuya/irrigation_locations",t);return await this._farm.refresh(!0),this._statsKey="",e}catch(t){const e=t;return this._error=e.body?.error??e.message??String(t),null}finally{this._busy=!1}}async _createPump(t){const e=this._sensorsOf(t),i=t=>e.find(e=>Te[t].test(e))??null,s=i("meter_entity");if(!s)return void(this._error=`${this._deviceName(t)} has no water meter sensor (…_fct_total_delivered_flow_mc). Create the pump from its meter's device.`);const n=await this._post({action:"create_pump",name:this._deviceName(t),meter_entity:s,flow_entity:i("flow_entity"),pressure_entity:i("pressure_entity"),status_entity:i("status_entity")}),a=n?.pump;a&&this._open(a.id)}_savePump(t,e){t.preventDefault();const i=t.target,s=t=>i.elements.namedItem(t).value,n={action:"update_pump",id:e.id,name:s("name")};for(const t of De)n[t]=s(t)||null;this._post(n)}_assign(t,e){t.preventDefault();const i=t.target.querySelector("select"),[s,n]=i.value.split(":");n&&this._post({action:"assign_pump",pump_id:e.id,target_kind:s,target_id:n})}_connect(t,e){t.preventDefault();const i=t.target,s=t=>i.elements.namedItem(t).value;this._post({action:"connect_device",pump_id:e.id,device_id:this._connecting,role:s("role"),meter_entity:s("meter_entity")||null}).then(t=>{t&&(this._connecting=null)})}_figures(t,e){const i=this._num(t.flow_entity),s=this._num(t.pressure_entity),n=this._state(t.status_entity)?.state,a=this._pumpState(t);return B`<div class="figures">
      ...
      >
    </div>`}_balance(t){const{ms:e,period:i}=Pe[this._range],s=Date.now(),n=function(t,e,i,s,n,a){const r=new Map,o=t=>{const e=Se(t,a);let i=r.get(e);return i||r.set(e,i={start:e,pump:null,valves:0,consumers:0}),i},l="hour"===a?36e5:864e5;for(let t=Se(s,a);t<n;t+=l)o(t);let d=null;for(const t of i[e.meter_entity]??[]){if(t.start<s||t.start>=n||"number"!=typeof t.change)continue;const e=o(t.start);e.pump=(e.pump??0)+t.change*ye,d=(d??0)+t.change*ye}let c=0;for(const i of t.runs){const a=Date.parse(i.end);if(a<s||a>=n||"number"!=typeof i.liters)continue;const r=t.locations.find(t=>t.assignments.some(t=>t.device_id===i.device_id&&we(t,a)));r&&ke(t,r,a)===e.id&&(c+=i.liters,o(Date.parse(i.start)).valves+=i.liters)}let p=0;for(const a of function(t,e,i,s){return t.pumpConnections.filter(t=>t.pump_id===e&&(null===t.begin||t.begin<s)&&(null===t.end||t.end>i))}(t,e.id,s,n))if("consumer"===a.role&&a.meter_entity)for(const t of i[a.meter_entity]??[])t.start<s||t.start>=n||"number"!=typeof t.change||!we(a,t.start)||(p+=t.change*ye,o(t.start).consumers+=t.change*ye);return{pump:d,valves:c,consumers:p,unaccounted:null===d?null:d-c-p,buckets:[...r.values()].sort((t,e)=>t.start-e.start)}}(this._farm.data,t,this._stats,s-e,s,i),a=t=>n.pump?` · ${Math.round(t/n.pump*100)} %`:"";return B`<section>
      ...
      <div class="tiles">
        <div title="What the pump's meter counted"><span class="dim">Pump delivered</span><b>${null===n.pump?"no data":Jt(n.pump)}</b></div>
        <div title="Runs of the valves this pump fed at the time"><span class="dim">Valves</span><b>${Jt(n.valves)}</b><span class="dim">${a(n.valves)}</span></div>
        <div title="Metered devices connected as consumers"><span class="dim">Metered consumers</span><b>${Jt(n.consumers)}</b><span class="dim">${a(n.consumers)}</span></div>
        <div title="Pump minus valves minus consumers: tanks, taps, unmetered valves, leaks">
          <span class="dim">Unaccounted</span><b>${null===n.unaccounted?"–":Jt(n.unaccounted)}</b
    ...
    </form>`}_valveDevices(){return this._farm.valves.map(t=>this.hass?.entities[t.registry_entity]?.device_id).filter(t=>!!t)}_pumpForm(t){const e=this._deviceOf(t.meter_entity),i=[...new Set([...this._sensorsOf(e),...De.map(e=>t[e]).filter(t=>!!t)])].sort();return B`<form class="editor" @submit=${e=>this._savePump(e,t)}>
      <label>Name <input name="name" .value=${t.name} required /></label>
      ${De.map(e=>B`<label
          >${ze[e]}
          <select name=${e}>
    ...
    >
      ...
      <main>
        <div class="title-row">
          <h2>${o?o.name:r===Ee?"No pump":"Pumps"}</h2>
        ...
        ${r===Ee?B`<p class="dim">Metering points no pump feeds: set a pump on their site or on them.</p>
              <div class="grid">${n.map(t=>B`<xt-mp-card .summary=${s.get(t.id)}></xt-mp-card>`)}</div>`:G}
      </main>
    </div>`}}Re.styles=[It,_e,r`
      section {
        margin-top: 24px;
      }
      .section-head {
    ...
    `],t([ht({attribute:!1})],Re.prototype,"hass",void 0),t([mt()],Re.prototype,"_selected",void 0),t([mt()],Re.prototype,"_range",void 0),t([mt()],Re.prototype,"_stats",void 0),t([mt()],Re.prototype,"_edit",void 0),t([mt()],Re.prototype,"_busy",void 0),t([mt()],Re.prototype,"_error",void 0),t([mt()],Re.prototype,"_connecting",void 0),customElements.get("irrigation-pumps-card")||customElements.define("irrigation-pumps-card",Re);
