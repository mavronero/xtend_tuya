...
def demo():
    ...
    assert lm.seed_from_names(d, [("dev835", "FF East Verbs 06 (835)")], T1, firsts.get)
    assert len(d["locations"]) == 1
    a835 = d["assignments"][0]
    assert a835["begin"] == firsts["dev835"] and a835["source"] == "auto"
    assert lm.seed_from_names(d, [("dev835", "FF East Verbs 06 (835)"), ("dev751", "FF East verbs 06 (751)")], T3, firsts.get)
    ...
    assert not lm.seed_from_names(d, [("dev751", "FF East verbs 06 (751)")], T4, firsts.get)
    assert len(open_of(d, "dev835")) == 1

    # --- same number, different places: keyed by device id -------------
    d = lm.empty()
    lm.seed_from_names(d, [("a", "HM verbs (701)"), ("b", "FG Herbs bed (701)")], T1, no_runs)
    assert len(d["locations"]) == 2 and len(d["assignments"]) == 2

    # --- SmartLife rename moves an auto assignment ----------------------
    d = lm.empty()
    lm.seed_from_names(d, [("x", "Old Bed (900)")], T1, no_runs)
    assert d["assignments"][0]["begin"] == T1  # no runs -> begin now
    assert lm.seed_from_names(d, [("x", "New Bed (900)")], T2, no_runs)
    ...
    assert not lm.seed_from_names(d, [("x", "New Bed (900)")], T4, no_runs)
    (cur,) = open_of(d, "x")
    ...
    d = lm.empty()
    lm.seed_from_names(d, [("y", "Plot A (1)")], T1, no_runs)
    loc_a = d["assignments"][0]["location_id"]
    lm.end_assignment(d, "y", loc_a, T2)
    assert not lm.seed_from_names(d, [("y", "Plot A (1)")], T3, no_runs)
    ...
    d = lm.empty()
    lm.seed_from_names(d, [("z", "HM Verbs (701)")], T1, no_runs)
    ...
    assert lm.seed_from_names(d, [("z", "HM Verbs (701)"), ("w", "hm verbs (903)")], T2, no_runs)  # w is new
    assert len(d["locations"]) == 1 and open_of(d, "w")[0]["location_id"] == loc["id"]
    assert lm.seed_from_names(d, [("v", "Herb Garden North (904)")], T2, no_runs) and len(d["locations"]) == 1
